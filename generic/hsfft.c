 ...
 *
 *  Adapted for VecTcl by Christian Gollwitzer (2014)
 */
...
fft_object fft_init(int N, int sgn) {
	...
	if (out == 1) {
		obj = (fft_object) malloc (sizeof(struct fft_set) + sizeof(NumArray_Complex)* (N-1));
		...
		obj = (fft_object) malloc (sizeof(struct fft_set) + sizeof(NumArray_Complex)* (M-1));
...
static void mixed_radix_dit_rec(NumArray_Complex *op,NumArray_Complex *ip,const fft_object obj, int sgn, int N,int l,int inc) {
...
static void bluestein_exp(NumArray_Complex *hl, NumArray_Complex *hlt, int len, int M) {
...
static void bluestein_fft(NumArray_Complex *data, NumArray_Complex *oup,fft_object obj,int sgn, int N) {
	obj->lt = 0;
	...
	NumArray_Complex* yn = (NumArray_Complex*) malloc (sizeof(NumArray_Complex) * M);
	NumArray_Complex* hk = (NumArray_Complex*) malloc (sizeof(NumArray_Complex) * M);
	NumArray_Complex* tempop = (NumArray_Complex*) malloc (sizeof(NumArray_Complex) * M);
	NumArray_Complex* yno = (NumArray_Complex*) malloc (sizeof(NumArray_Complex) * M);
    NumArray_Complex* hlt = (NumArray_Complex*) malloc (sizeof(NumArray_Complex) * N);
    //NumArray_Complex* twi = (NumArray_Complex*) malloc (sizeof(NumArray_Complex) * M);
